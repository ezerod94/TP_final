...
```{r include=FALSE}
# creo capa de entrada que va a ser común a todos los modelos
...
model_trad <- keras_model(inputs=inputs, outputs= capas)
...
model_trad<- keras::load_model_weights_hdf5(model_trad, "model_trad.hdf5")
```
...
# calculos los siguientes cuantiles y los guardo en un data.frame
...
dist <- inputs %>%
  layer_dense_flipout(units = 20,
                      ...
                      activation="relu") %>%
  layer_dense_flipout(units = 50,    bias_posterior_fn = tfp$layers$util$default_mean_field_normal_fn(),
                                  bias_prior_fn = tfp$layers$util$default_multivariate_normal_fn,
                                  kernel_divergence_fn = kl_div,
                                  bias_divergence_fn = kl_div,
                                  activation="relu") %>%
  layer_dense_flipout(units = 20,    bias_posterior_fn = tfp$layers$util$default_mean_field_normal_fn(),
                                  bias_prior_fn = tfp$layers$util$default_multivariate_normal_fn,
                                  kernel_divergence_fn = kl_div,
                                  bias_divergence_fn = kl_div,
                                  activation="relu") %>%
  layer_dense_flipout(units = 2,     bias_posterior_fn = tfp$layers$util$default_mean_field_normal_fn(),
                                  bias_prior_fn = tfp$layers$util$default_multivariate_normal_fn,
                                  kernel_divergence_fn = kl_div,
                                  bias_divergence_fn = kl_div,
                                  name = 'params_vi') %>%
...
params <- keras_model(inputs=inputs, outputs= model_vi$get_layer('params_vi')$output)
model_vi  %>% compile(optimizer=optimizer_adam(learning_rate = 0.001), loss = negloglik) 
...
gg3 <- ggplot() +
  ...
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))+ ylim(-2, 6)+ xlim(-5,25)
...
dist_mc <- inputs %>%
             ...
             layer_dense(units = 200, activation="relu") %>%
             layer_dropout(rate = 0.1, trainable = T) %>%
             layer_dense(units = 2, name = 'params_mc') %>% 
             layer_distribution_lambda(function(x)
                            tfd_normal(loc = x[, 1, drop = FALSE],
...
gg2 <- gg2 + ggtitle(label = "IC 95") + theme(plot.title = element_text(size = 10)) + labs(subtitle = NULL)
gg3 <- gg3 + ggtitle(label = "IC 95") + theme(plot.title = element_text(size = 10)) + labs(subtitle = NULL)
gg4 <- gg4 + ggtitle(label = "IC 95") + theme(plot.title = element_text(size = 10)) + labs(subtitle = NULL)
